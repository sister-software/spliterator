...
**`CharacterSequence`** (`lib/CharacterSequence.ts`) — Encodes a delimiter string/bytes and provides a `search(source, start, end?)` method for scanning byte arrays. Defines the `Delimiters` enum (Newline, Comma, Tab, etc.).
...
- [ ] **WASM SIMD for multi-byte delimiter scanning**
  - The `CharacterSequence.search` fast path covers single-byte delimiters via `Uint8Array.indexOf` (V8 uses SSE2/NEON internally). Multi-byte delimiters (CRLF, custom sequences) still fall back to a JS loop. A small Rust or hand-written WAT module exporting `findDelimiter(ptr, len, pattern)` using `i8x16.eq` + `i8x16.bitmask` (WASM SIMD, supported in Node ≥ 16) could approach ripgrep-level throughput (~10 GB/s). Integration point: `CharacterSequence.search` for `this.length > 1`.
